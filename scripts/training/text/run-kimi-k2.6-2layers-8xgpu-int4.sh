...
#
# Kimi K2.6 2-layer 8xGPU text-only colocate training, INT4 QAT (single-node smoke test).
#
...
#
# Model placement (TP=2 PP=1 CP=2 DP=2 EP=8 ETP=1) fits a single 8-GPU node;
# EP*ETP*EDP=8 equals TP*CP*DP=8. PP is kept at 1 (2 layers make PP degenerate).
# SGLang inference uses 8 GPUs per engine with DP-attention (dp_size=8, ep_size=8).
#
...
#
# Usage:
#   bash scripts/training/text/run-kimi-k2.6-2layers-8xgpu-int4.sh
...
# Reduce the model to 2 layers for this single-node smoke test (default is 61).
# MUST match num_hidden_layers in the checkpoint config.json; a
# mismatch makes Megatron build extra randomly-initialized layers, blowing up the
# train-infer logprob diff.
NLAYERS=2
source "${MODEL_CONFIG_DIR}/kimi-k2.6.sh"

PROJECT_NAME="${PROJECT_NAME:=Relax/dev/kimi-k2.6-2layers-8xgpu-text-int4}"
...
CKPT_ARGS=(
   ...
   --save ${EXP_DIR}/Kimi-K2.6_text_2layers_8xgpu_int4_ckpt/
   --save-interval 50
   --no-save-optim
   --no-save-rng
   --no-load-optim
...
WANDB_ARGS=(
   ...
   --tb-experiment-name Kimi-K2.6-text-2layers-8xgpu-int4-${now}
)
...
stdbuf -oL -eL ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://127.0.0.1:8265" \
   ...
   "${PERF_ARGS[@]}" \
   "${SGLANG_ARGS[@]}" \
   "${MISC_ARGS[@]}"  2>&1 | tee log/Kimi-K2.6-text-2layers-8xgpu-int4-${now}.log
